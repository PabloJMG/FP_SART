...
void elipse(int dimx, int dimy, int factordim, double *obj, float val, float x0, float y0, float ax, float bx);
...
#define INPUT_TXT
#define PARALLEL_BEAM //RAND_ANG || DEB_ANG || PARALLEL_BEAM
#define PHANTOM
#define DOSPI/*La otra opcion es abanico*/
//~ #define ABSORCION_TOTAL
...
angmax=anchura_malla/(2*(anchura_malla+d)); 
	...
	deltax=par[0].factordim*par[0].dix/10;
	deltay=par[0].factordim*par[0].diy/10;
	radio=deltay/2;
									...
									*(objeto+a*par[0].factordim*par[0].diy+b)=0.02;
											...
											b < par[0].factordim*par[0].diy/2 +deltay && b > par[0].factordim*par[0].dix/2 - deltay) /*Aqui parentesis en las condiciones ¿? */
												*(objeto+a*par[0].factordim*par[0].diy+b)=50;
														//~ printf("Flor");
														//~ getchar();}
									fprintf(finp, "%d %d %lf\n", a, b, *(objeto+a*par[0].factordim*par[0].diy+b));
							    }
	...
	dimtotal=par[0].factordim*par[0].dix*par[0].factordim*par[0].diy;
			...
			for(a=0; a<NElip; a++)
			{
				elipse(par[0].dix, par[0].diy, par[0].factordim, objeto, val[a], x0[a], y0[a], ax[a], bx[a]); /*Elipse no está aquí !!! x0, y0 puntos iniciales, val el valor, ax, bx semiejes mayor y menor */
				
			}
	
			finp=fopen("../res/matriz_objeto.txt", "w");
			
			for(a=0; a<par[0].dix*par[0].factordim; a++)
			{
				for(b=0; b<par[0].diy*par[0].factordim; b++)
				fprintf(finp, "%d %d %f\n", a, b, *(objeto+b+a*par[0].diy*par[0].factordim));
						...
						{		
							*(objeto+a*par[0].factordim*par[0].diy+b)=0.02;
								...
								*(objeto+a*par[0].factordim*par[0].diy+b)=1;
								...
								if(((a-par[0].factordim*par[0].dix/2)* (a-par[0].factordim*par[0].dix/2) + (b - par[0].factordim*par[0].diy/2)*(b - par[0].factordim*par[0].diy/2)) < (radio*radio))
								*(objeto + a*par[0].factordim*par[0].diy + b)=200;
...
void guarda_array(double *array, int dimx, int dimy, FILE *fau) //Sólo usado dentro de trata_objeto, función que además no se utiliza
{	int i, j;
...
char nombrearchivo[100]="../res/Resultados_pixeles.txt";
#ifdef DOSPI
...
void *propagacion(void *threadarg) /*Falta lo del objeto*/
{
	...
	//~ int *resultados=crea_array_ent(NumPi);
	int *resultados=calloc(NumPi, sizeof(int));
...
for(i=0; i<NRayos; i++) /*Iteracion de todos los rayos para un mismo angulo*/
{
	...
	*pos0 = factor*anchura/2-deltadis+ 2*deltadis*num_rand; /*Fuente extensa, posicion y aleatoria*/
	 *(pos0+1)= betarnd;
	 ...
	 #ifdef PARALLEL_BEAM
	 betarnd=0;
	 num_rand=(float)i/NRayos;

	 *pos0=(factor-1)*anchura/2+anchura*num_rand;
	 *(pos0+1)=betarnd;
	 #endif
...
printf("El número de píxel es %d y pos del rayo es %lf, i %d mu =%lf y angulo %d, mientras, no han llegado %d rayos\n", num_pix, *pos0-factor*anchura/2+anchura/2, i,  mu, cont, pami);
pami=0;
...
void elipse(int dimx, int dimy, int factordim, double *obj, float val, float x0, float y0, float ax, float bx)
{
	...
	for(cont=0; cont<(factordim*dimx); cont++)
	{
		...
		for(cont2=0; cont2<(factordim*dimy); cont2++)
		{
			if(((cont-x0-factordim*dimx/2)*(cont-x0-factordim*dimx/2)/(ax*ax)+(cont2-y0-factordim*dimy/2)*(cont2-y0-factordim*dimy/2)/(bx*bx))<1.0)
			*(obj + cont2 + cont*factordim*dimy)+=val;
		}
